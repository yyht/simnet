...
from model.utils.dsmm.inputs.dynamic_pooling import dynamic_pooling_index
...
class BCNNBaseModel(BaseModel):
    def __init__(self, *args, **kargs):
        super(BCNNBaseModel, self).__init__(*args, **kargs)


    def build_placeholder(self, config):
        super(BCNNBaseModel, self).build_placeholder(config)
        with self.graph.as_default():
            self.dpool_index_word = tf.placeholder(tf.int32, shape=[None, self.config["max_seq_len_word"],
                                                self.config["max_seq_len_word"], 3],
                                               name="dpool_index_word")
        # self.dpool_index_char = tf.placeholder(tf.int32, shape=[None, self.config["max_seq_len_char"],
        #                                                         self.config["max_seq_len_char"], 3],
    ...
    def _padding(self, x, name):
        # x: [batch, s, d, 1]
        # x => [batch, s+w*2-2, d, 1]
        w = self.config["bcnn_filter_size"]
    ...
    def _make_attention_matrix(self, x1, x2):
        ...
        if "euclidean" in self.config["bcnn_match_score_type"]:
            ...
            if "exp" in self.config["bcnn_match_score_type"]:
                ...
                att = 1. / (1. + euclidean)
        elif self.config["bcnn_match_score_type"] == "cosine":
            # cosine similarity
    ...
    def _convolution(self, x, d, name, reuse=False):
        ...
        print("_conv x shape===", x.get_shape())
        conv = tf.layers.conv2d(
            inputs=x,
            filters=self.config["bcnn_num_filters"],
            kernel_size=(self.config["bcnn_filter_size"], d),
            padding="valid",
            activation=get_activation(self.config["bcnn_activation"]),
            strides=1,
            reuse=reuse,
            name=name)

        print("==_conv shape==", conv.get_shape())
    ...
    def _w_ap(self, x, attention, name):
        ...
        w_ap = tf.layers.average_pooling2d(
            inputs=x2,
            pool_size=(self.config["bcnn_filter_size"], 1),
            strides=1,
            padding="valid",
            name=name)
        if attention is not None:
            w_ap = w_ap * self.config["bcnn_filter_size"]
    ...
    def _all_ap(self, x, seq_len, name):
        if "input" in name:
            pool_width = seq_len
            d = self.emb_size #self.config["embedding_dim"]
        else:
            pool_width = seq_len + self.config["bcnn_filter_size"] - 1
            d = self.config["bcnn_num_filters"]
    ...
    def _mp_cnn_layer(self, cross, dpool_index, filters, kernel_size, pool_size, strides, name):
        cross_conv = tf.layers.conv2d(
            ...
            padding="same",
            activation=get_activation(self.config["bcnn_mp_activation"]),
            strides=1,
            reuse=False,
            name=name+"cross_conv")
        if self.config["bcnn_mp_dynamic_pooling"] and dpool_index is not None:
    ...
    def _bcnn_semantic_feature_layer(self, seq_left, seq_right, dpool_index=None, granularity="word"):
        ...
        seq_len = self.config["max_seq_len_%s" % granularity]

        # [batch, s, d] => [batch, s, d, 1]
        seq_left = tf.expand_dims(seq_left, axis=-1)
        seq_right = tf.expand_dims(seq_right, axis=-1)

        left_ap_list = [None] * (self.config["bcnn_num_layers"] + 1)
        right_ap_list = [None] * (self.config["bcnn_num_layers"] + 1)
        ...
        print("==initial x1 x2 shape===", x1.get_shape(),x2.get_shape())

        d = self.emb_size #self.config["embedding_dim"]
        outputs = []
        for layer in range(self.config["bcnn_num_layers"]):
            ...
            d = self.config["bcnn_num_filters"]
            if self.config["bcnn_mp_att_pooling"] and att_pooled is not None:
                outputs.append(att_pooled)

        for l, r in zip(left_ap_list, right_ap_list):
            outputs.append(metrics.cosine_similarity(l, r, self.config["similarity_aggregation"]))
            outputs.append(metrics.dot_product(l, r, self.config["similarity_aggregation"]))
            outputs.append(metrics.euclidean_distance(l, r, self.config["similarity_aggregation"]))
        outputs = tf.concat(outputs, axis=-1)
        print(outputs.get_shape(), "===bcnn semantic feature===")
        return outputs
    ...
    def _get_attention_matrix_pooled_features(self, att_mat, seq_len, dpool_index, granularity, name):
        ...
        # conv-pool layer 1
        filters = self.config["bcnn_mp_num_filters"][0]
        kernel_size = self.config["bcnn_mp_filter_sizes"][0]
        # seq_len = seq_len + self.config["bcnn_filter_size"] - 1
        pool_size0 = self.config["bcnn_mp_pool_sizes_%s" % granularity][0]
        ...
        conv1_flatten = tf.reshape(conv1, [-1, self.config["mp_num_filters"][0] * (pool_size0 * pool_size0)])

        # conv-pool layer 2
        filters = self.config["bcnn_mp_num_filters"][1]
        kernel_size = self.config["bcnn_mp_filter_sizes"][1]
        pool_size1 = self.config["bcnn_mp_pool_sizes_%s" % granularity][1]
...
class BCNN(BCNNBaseModel):
    def __init__(self, *args, **kargs):
        super(BCNN, self).__init__(*args, **kargs)
    ...
    def _bcnn_cnn_layer(self, x1, x2, seq_len, d, name, dpool_index=None, granularity="word"):
        ...
        # get attention matrix pooled features (as in sec. 5.3.1)
        print(left_ap.get_shape(), left_wp.get_shape(),
            right_wp.get_shape(), right_ap.get_shape(),
            att_mat0.get_shape())
...
class ABCNN1(BCNNBaseModel):
    def __init__(self, *args, **kargs):
        super(ABCNN1, self).__init__(*args, **kargs)
...
class ABCNN2(BCNNBaseModel):
    def __init__(self, *args, **kargs):
        super(ABCNN2, self).__init__(*args, **kargs)
    # def __init__(self, params, logger, init_embedding_matrix):
    #     p = copy(params)
...
class ABCNN3(BCNNBaseModel):
    def __init__(self, *args, **kargs):
        super(ABCNN3, self).__init__(*args, **kargs)
    # def __init__(self, params, logger, init_embedding_matrix):
    #     p = copy(params)
