...
class MatchPyramidBaseModel(BaseModel):
    def __init__(self, *args, **kargs):
        super(MatchPyramidBaseModel, self).__init__(*args, **kargs)

    def build_placeholder(self, config):
        super(MatchPyramidBaseModel, self).build_placeholder(config)
    ...
    def _get_match_matrix(self, seq_left, emb_seq_left, enc_seq_left, seq_right, emb_seq_right, enc_seq_right,
        ...
        ew_shape0 = self.emb_size #self.config["embedding_dim_%s" % granularity
        ...
        match_matrix_element_concat_com = tf.concat([
            tf.tile(tf.expand_dims(emb_seq_com_left, 2), [1, 1, self.config["max_seq_len_%s" % granularity], 1]),
            tf.tile(tf.expand_dims(emb_seq_com_right, 1), [1, self.config["max_seq_len_%s" % granularity], 1, 1]),
        ], axis=-1)
        ...
        match_matrix_element_concat_ctx = tf.concat([
            tf.tile(tf.expand_dims(enc_seq_left, 2), [1, 1, self.config["max_seq_len_%s" % granularity], 1]),
            tf.tile(tf.expand_dims(enc_seq_right, 1), [1, self.config["max_seq_len_%s" % granularity], 1, 1]),
        ], axis=-1)
    ...
    def _mp_cnn_layer(self, cross, dpool_index, filters, kernel_size, pool_size, strides, name):
        cross_conv = tf.layers.conv2d(
            ...
            padding="same",
            activation=get_activation(self.config["mp_activation"]),
            strides=1,
            reuse=False,
            name=name+"cross_conv")
        if self.config["mp_dynamic_pooling"] and dpool_index is not None:
    ...
    def _mp_semantic_feature_layer(self, match_matrix, dpool_index, granularity="word"):
        ...
        # conv-pool layer 1
        filters = self.config["mp_num_filters"][0]
        kernel_size = self.config["mp_filter_sizes"][0]
        seq_len = self.config["max_seq_len_%s" % granularity]
        pool_size0 = self.config["mp_pool_sizes_%s" % granularity][0]
        ...
        # conv-pool layer 2
        filters = self.config["mp_num_filters"][1]
        kernel_size = self.config["mp_filter_sizes"][1]
        pool_size1 = self.config["mp_pool_sizes_%s" % granularity][1]
    ...
    def _get_feed_dict(self, Q, training=False, symmetric=False):
        feed_dict = super(MatchPyramidBaseModel, self)._get_feed_dict(Q, training, symmetric)
...
class MatchPyramid(MatchPyramidBaseModel):
    def __init__(self, *args, **kargs):
        super(MatchPyramid, self).__init__(*args, **kargs)
...
class GMatchPyramid(MatchPyramidBaseModel):
    def __init__(self, *args, **kargs):
        super(GMatchPyramid, self).__init__(*args, **kargs)

    def build_placeholder(self, config):
        config = copy(config)
        config.update({
            "model_name": "g_match_pyramid",
        ...
        super(GMatchPyramid, self).build_placeholder(config)
